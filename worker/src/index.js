...
const SYSTEM_PROMPT = `You are a research assistant for a marine geophysicist named Maleen.
...
export default {
  async fetch(req, env) {
    ...
    if (req.method === 'GET' && url.pathname === '/') {
      return new Response('notes-rag worker ok', { status: 200, headers: cors });
    }
    ...
    if (!env.CHAT_PASSWORD || !timingSafeEqual(String(password || ''), env.CHAT_PASSWORD)) {
      return json({ error: 'unauthorized' }, 401, cors);
    }
