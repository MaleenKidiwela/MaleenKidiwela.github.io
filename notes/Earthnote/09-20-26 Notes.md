...
During ingestion, each corpus pipeline creates explicit links from its source material.

  For example, an instrument catalog record already says things like:
  ...
  The normalizer assigns stable IDs, such as:

  instrument: RS03INT2-MJ03D-12-VEL3DB304
  site: Axial International District
  parameter: seawater velocity
  system: OOI M2M

  It then writes explicit edge facts:

  instrument → deployed_at → site
  instrument → measures → parameter
  instrument → accessible_through → system
  instrument → documented_by → source

  Text chunks are linked to the same IDs through metadata. So a VEL3D paragraph, its instrument record, the Axial site record,
  and its OOI data-access page all meet at shared stable entities.

  Some edges come directly from structured catalogs; others come from carefully normalized source references, citations,
  deployment tables, tool schemas, and provenance files. It is not connecting nodes merely because two words look similar.
